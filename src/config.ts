...
class Config {
  ...
  get customBranchGlobPatterns(): CustomBranchGlobPattern[] {
  	...
  	for (let i = 0; i < inPatterns.length; i++) {
  		if (
  			typeof inPatterns[i].name === 'string' &&
        typeof inPatterns[i].glob === 'string'
  		) {
  			outPatterns.push({
  				name: inPatterns[i].name,
  				glob: '--glob=' + inPatterns[i].glob
  			});
  		}
  	}
  	return outPatterns;
  ...
  get customEmojiShortcodeMappings(): CustomEmojiShortcodeMapping[] {
  	...
  	for (let i = 0; i < inMappings.length; i++) {
  		if (
  			typeof inMappings[i].shortcode === 'string' &&
        typeof inMappings[i].emoji === 'string'
  		) {
  			outMappings.push({
  				shortcode: inMappings[i].shortcode,
  				emoji: inMappings[i].emoji
  			});
  		}
  	}
  	return outMappings;
  ...
  get customPullRequestProviders(): CustomPullRequestProvider[] {
  	...
  	return Array.isArray(providers)
  		? providers
              ...
              typeof provider.templateUrl === 'string'
  			)
  			.map((provider) => ({
  				name: provider.name,
   ...
   */
  get dateType() {
  	return this.getRenamedExtensionSetting<string>(
  		'date.type',
  		'dateType',
  		'Author Date'
  	) === 'Commit Date'
  ...
  get dialogDefaults(): DialogDefaults {
  	...
  	return {
  		addTag: {
  			...
  			type:
          this.config.get<string>('dialog.addTag.type', 'Annotated') ===
          'Lightweight'
          	? TagType.Lightweight
          	: TagType.Annotated
  		},
  		applyStash: {
  			reinstateIndex: !!this.config.get(
  		...
  		deleteBranch: {
  			forceDelete: !!this.config.get(
  				'dialog.deleteBranch.forceDelete',
  				false
  			)
  		},
  		fetchIntoLocalBranch: {
  			forceFetch: !!this.config.get(
  ...
  get graph(): GraphConfig {
  	const colours = this.getRenamedExtensionSetting<string[]>(
  		'graph.colours',
  		'graphColours',
  		[]
  	);
  ...
  get openNewTabEditorGroup(): vscode.ViewColumn {
      ...
      typeof VIEW_COLUMN_MAPPING[location] !== 'undefined'
  ...
  get onRepoLoad(): OnRepoLoadConfig {
  	const branches = this.config.get(
  ...
  get repoDropdownOrder(): RepoDropdownOrder {
  	const order = this.config.get<string>(
   ...
   */
  private getRenamedExtensionSetting<T>(
  	newSection: string,
  	oldSection: string,
  	defaultValue: T
  ) {
      ...
      typeof newValues.workspaceValue !== 'undefined'
  	)
  		return newValues.workspaceValue;
  	if (
  		typeof oldValues !== 'undefined' &&
      typeof oldValues.workspaceValue !== 'undefined'
  	)
  		return oldValues.workspaceValue;
  	if (
  		typeof newValues !== 'undefined' &&
      typeof newValues.globalValue !== 'undefined'
  	)
  		return newValues.globalValue;
  	if (
  		typeof oldValues !== 'undefined' &&
      typeof oldValues.globalValue !== 'undefined'
  	)
