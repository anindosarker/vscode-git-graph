...
export class GitGraphView extends Disposable {
  ...
  public static createOrShow(
  	...
  	if (GitGraphView.currentPanel) {
  		...
  		if (GitGraphView.currentPanel.isPanelVisible) {
  			...
  			if (loadViewTo !== null) {
  				GitGraphView.currentPanel.respondLoadRepos(
  ...
  private constructor(
  	...
  	this.registerDisposables(
  		...
  		repoManager.onDidChangeRepos((event) => {
  			...
  			const loadViewTo =
          event.loadRepo !== null ? { repo: event.loadRepo } : null;
  			if (
  				(event.numRepos === 0 && this.isGraphViewLoaded) ||
          (event.numRepos > 0 && !this.isGraphViewLoaded)
  			) {
  				this.loadViewTo = loadViewTo;
  				this.update();
  	...
  	this.logger.log(
  		'Created Git Graph View' +
        (loadViewTo !== null ? ' (active repo: ' + loadViewTo.repo + ')' : '')
  	);
  }
  ...
  private async respondToMessage(msg: RequestMessage) {
  	...
  	switch (msg.command) {
  		case 'addRemote':
  			this.sendMessage({
  				command: 'addRemote',
  				error: await this.dataSource.addRemote(
  					msg.repo,
  					msg.name,
  					msg.url,
  					msg.pushUrl,
  		...
  		case 'applyStash':
  			this.sendMessage({
  				command: 'applyStash',
  				error: await this.dataSource.applyStash(
  					msg.repo,
  					msg.selector,
  					msg.reinstateIndex
  				)
  		...
  		case 'branchFromStash':
  			this.sendMessage({
  				...
  				error: await this.dataSource.branchFromStash(
  					msg.repo,
  					msg.selector,
  					msg.branchName
  				)
  		...
  		case 'checkoutBranch':
  			errorInfos = [
  				await this.dataSource.checkoutBranch(
  					msg.repo,
  					msg.branchName,
  					msg.remoteBranch
  				)
  		...
  		case 'cleanUntrackedFiles':
  			this.sendMessage({
  				...
  				error: await this.dataSource.cleanUntrackedFiles(
  					msg.repo,
  					msg.directories
  				)
  			});
  		...
  		case 'commitDetails':
  			let data = await Promise.all([
  				msg.commitHash === UNCOMMITTED
  					...
  					: msg.stash === null
  						? this.dataSource.getCommitDetails(
  							msg.repo,
  							msg.commitHash,
  							msg.hasParents
  						)
  						: this.dataSource.getStashDetails(
  							msg.repo,
  							msg.commitHash,
  							msg.stash
  						),
  		...
  		case 'compareCommits':
  			this.sendMessage({
  				...
  				...(await this.dataSource.getCommitComparison(
  					msg.repo,
  					msg.fromHash,
  					msg.toHash
  				)),
            ...
            msg.toHash !== UNCOMMITTED
            	? this.extensionState.getCodeReview(
            		msg.repo,
            		msg.fromHash + '-' + msg.toHash
            	)
            	: null,
  				refresh: msg.refresh
  			});
  		...
  		case 'copyFilePath':
  			this.sendMessage({
  				...
  				error: await copyFilePathToClipboard(
  					msg.repo,
  					msg.filePath,
  					msg.absolute
  				)
  		...
  		case 'createBranch':
  			this.sendMessage({
  				...
  				errors: await this.dataSource.createBranch(
  					msg.repo,
  					msg.branchName,
  					msg.commitHash,
  					msg.checkout,
  		...
  		case 'deleteBranch':
  			errorInfos = [
  				await this.dataSource.deleteBranch(
  					msg.repo,
  					msg.branchName,
  					msg.forceDelete
  				)
  			...
  			if (errorInfos[0] === null) {
  				for (let i = 0; i < msg.deleteOnRemotes.length; i++) {
  					errorInfos.push(
  						await this.dataSource.deleteRemoteBranch(
  							msg.repo,
  							msg.branchName,
  							msg.deleteOnRemotes[i]
  						)
  					);
  				}
  			}
  		...
  		case 'deleteRemoteBranch':
  			this.sendMessage({
  				...
  				error: await this.dataSource.deleteRemoteBranch(
  					msg.repo,
  					msg.branchName,
  					msg.remote
  				)
  		...
  		case 'deleteTag':
  			this.sendMessage({
  				command: 'deleteTag',
  				error: await this.dataSource.deleteTag(
  					msg.repo,
  					msg.tagName,
  					msg.deleteOnRemote
  				)
  		...
  		case 'deleteUserDetails':
  			...
  			if (msg.name) {
  				errorInfos.push(
  					await this.dataSource.unsetConfigValue(
  						msg.repo,
  		...
  		case 'editUserDetails':
  			errorInfos = [
  				await this.dataSource.setConfigValue(
  					msg.repo,
  					GitConfigKey.UserName,
  					msg.name,
  					msg.location
  		...
  		case 'fetch':
  			this.sendMessage({
  				command: 'fetch',
  				error: await this.dataSource.fetch(
  					msg.repo,
  					msg.name,
  					msg.prune,
  					msg.pruneTags
  		...
  		case 'openExternalDirDiff':
  			this.sendMessage({
  				...
  				error: await this.dataSource.openExternalDirDiff(
  					msg.repo,
  					msg.fromHash,
  					msg.toHash,
  					msg.isGui
  		...
  		case 'openTerminal':
  			this.sendMessage({
  				...
  				error: await this.dataSource.openGitTerminal(
  					msg.repo,
  					null,
  					msg.name
  				)
  		...
  		case 'popStash':
  			this.sendMessage({
  				command: 'popStash',
  				error: await this.dataSource.popStash(
  					msg.repo,
  					msg.selector,
  					msg.reinstateIndex
  				)
  		...
  		case 'pushStash':
  			this.sendMessage({
  				command: 'pushStash',
  				error: await this.dataSource.pushStash(
  					msg.repo,
  					msg.message,
  					msg.includeUntracked
  				)
  			});
  			break;
  		case 'pushTag':
  			this.sendMessage({
  				...
  				remotes: msg.remotes,
  				commitHash: msg.commitHash,
  				errors: await this.dataSource.pushTag(
  					msg.repo,
  					msg.tagName,
  					msg.remotes,
  					msg.commitHash,
  		...
  		case 'renameBranch':
  			this.sendMessage({
  				...
  				error: await this.dataSource.renameBranch(
  					msg.repo,
  					msg.oldName,
  					msg.newName
  				)
  		...
  		case 'resetFileToRevision':
  			this.sendMessage({
  				...
  				error: await this.dataSource.resetFileToRevision(
  					msg.repo,
  					msg.commitHash,
  					msg.filePath
  				)
  		...
  		case 'resetToCommit':
  			this.sendMessage({
  				...
  				error: await this.dataSource.resetToCommit(
  					msg.repo,
  					msg.commit,
  					msg.resetMode
  				)
  		...
  		case 'revertCommit':
  			this.sendMessage({
  				...
  				error: await this.dataSource.revertCommit(
  					msg.repo,
  					msg.commitHash,
  					msg.parentIndex
  				)
  		...
  		case 'startCodeReview':
  			this.sendMessage({
  				...
  				commitHash: msg.commitHash,
  				compareWithHash: msg.compareWithHash,
  				...(await this.extensionState.startCodeReview(
  					msg.repo,
  					msg.id,
  					msg.files,
  					msg.lastViewedFile
  		...
  		case 'updateCodeReview':
  			this.sendMessage({
  				...
  				error: await this.extensionState.updateCodeReview(
  					msg.repo,
  					msg.id,
  					msg.remainingFiles,
  					msg.lastViewedFile
  		...
  		case 'viewDiffWithWorkingFile':
  			this.sendMessage({
  				...
  				error: await viewDiffWithWorkingFile(
  					msg.repo,
  					msg.hash,
  					msg.filePath,
  					this.dataSource
  ...
  private sendMessage(msg: ResponseMessage) {
  	if (this.isDisposed()) {
  		this.logger.log(
  			'The Git Graph View has already been disposed, ignored sending "' +
          msg.command +
          '" message.'
  		);
  	} else {
  		this.panel.webview.postMessage(msg).then(
  			...
  			() => {
  				if (this.isDisposed()) {
  					this.logger.log(
  						'The Git Graph View was disposed while sending "' +
                msg.command +
                '" message.'
  					);
  				} else {
  ...
  private getHtmlForWebview() {
  	...
  	for (let i = 0; i < initialState.config.graph.colours.length; i++) {
        ...
        i +
        ':' +
        initialState.config.graph.colours[i] +
        '; ';
  		colorParams +=
        '[data-color="' +
        i +
        '"]{--git-graph-color:var(--git-graph-color' +
        i +
        ');} ';
  	}
  	...
  	if (this.dataSource.isGitExecutableUnknown()) {
  		...
  		body = `<body>
			...
			<script nonce="${nonce}">var initialState = ${JSON.stringify(
	initialState
)}, globalState = ${JSON.stringify(
	globalState
)}, workspaceState = ${JSON.stringify(workspaceState)};</script>
  	...
  	return `<!DOCTYPE html>
		<html lang="en">
			<head>
				<meta charset="UTF-8">
				<meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src ${standardiseCspSource(
		this.panel.webview.cspSource
	)} 'unsafe-inline'; script-src 'nonce-${nonce}'; img-src data:;">
				<meta name="viewport" content="width=device-width, initial-scale=1.0">
				<link rel="stylesheet" type="text/css" href="${this.getMediaUri(
		'out.min.css'
	)}">
...
export function standardiseCspSource(cspSource: string) {
	if (cspSource.startsWith('http://') || cspSource.startsWith('https://')) {
		...
		if (
			queryIndex > -1 &&
      (queryIndex < endOfAuthorityIndex || endOfAuthorityIndex === -1)
		)
			endOfAuthorityIndex = queryIndex;
		if (
			fragmentIndex > -1 &&
      (fragmentIndex < endOfAuthorityIndex || endOfAuthorityIndex === -1)
		)
			endOfAuthorityIndex = fragmentIndex;
		return endOfAuthorityIndex > -1
			? cspSource.substring(0, endOfAuthorityIndex)
			: cspSource;
	} else {
		return cspSource;
	}
