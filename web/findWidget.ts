...
class FindWidget {
  ...
  constructor(view: GitGraphView) {
  	...
  	alterClass(
  		this.caseSensitiveElem,
  		CLASS_ACTIVE,
  		workspaceState.findIsCaseSensitive
  	);
  ...
  private findMatches(goToCommitHash: string | null, scrollToCommit: boolean) {
  	...
  	if (this.text !== '') {
  		...
  		try {
  			...
  			this.widgetElem.setAttribute(ATTR_ERROR, (e as any).message);
  		}
  		if (findPattern !== null && findGlobalPattern !== null) {
            ...
            ((colVisibility.author && findPattern.test(commit.author)) ||
              ...
              branchLabels.remotes.some((remote) =>
                findPattern!.test(remote.name)
              ) ||
              commit.tags.some((tag) => findPattern!.test(tag.name)) ||
              (colVisibility.date &&
                findPattern.test(formatShortDate(commit.date).formatted)) ||
              (commit.stash !== null &&
                findPattern.test(commit.stash.selector)))
  				) {
  					let idStr = i.toString();
  					while (
  						j < commitElems.length &&
              commitElems[j].dataset.id !== idStr
  					)
  						j++;
                  ...
                  textElem.parentNode!.insertBefore(
                  	document.createTextNode(
                  		text.substring(matchEnd, match.index)
                  	),
                  	textElem
                  );
  			...
  			if (zeroLengthMatch) {
  				this.widgetElem.setAttribute(
  					ATTR_ERROR,
  					'Cannot use a regular expression which has zero length matches'
  				);
  				this.clearMatches();
  				this.matches = [];
  			}
  	...
  	if (this.matches.length > 0) {
  		...
  		if (goToCommitHash !== null) {
  			let pos = this.matches.findIndex(
  				(match) => match.hash === goToCommitHash
  			);
  			if (pos > -1) newPos = pos;
  		}
  	}
  ...
  private clearMatches() {
  	for (let i = 0; i < this.matches.length; i++) {
  		...
  		let matchElems = getChildrenWithClassName(
  				this.matches[i].elem,
  				CLASS_FIND_MATCH
  			),
  			matchElem;
        ...
        matchElem.parentNode!.replaceChild(
  ...
  private updatePosition(position: number, scrollToCommit: boolean) {
  	if (this.position > -1)
  		this.matches[this.position].elem.classList.remove(
      ...
      this.matches.length > 0
      	? this.position + 1 + ' of ' + this.matches.length
      	: 'No Results';
  	this.view.saveState();
  }
  ...
  private prev() {
  	...
  	this.updatePosition(
  		this.position > 0 ? this.position - 1 : this.matches.length - 1,
  		true
  	);
  	this.openCommitDetailsViewForCurrentMatchIfEnabled();
  }
  ...
  private next() {
  	if (this.matches.length === 0) return;
  	this.updatePosition(
  		this.position < this.matches.length - 1 ? this.position + 1 : 0,
  		true
  	);
  	this.openCommitDetailsViewForCurrentMatchIfEnabled();
  }
