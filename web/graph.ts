...
class Branch {
  ...
  public draw(svg: SVGElement, config: GG.GraphConfig, expandAt: number) {
    ...
    for (i = 0; i < lines.length; i++) {
      ...
      if (
        curPath === "" ||
        (i > 0 && (x1 !== lines[i - 1].p2.x || y1 !== lines[i - 1].p2.y))
      )
  ...
  private static drawPath(
    ...
    if (
      !isCommitted &&
      uncommittedChanges ===
        GG.GraphUncommittedChangesStyle.OpenCircleAtTheCheckedOutCommit
    ) {
...
class Vertex {
  ...
  public draw(
    ...
    if (avatar !== null) {
      ...
      image.setAttribute("x", (parseFloat(cx) - 10).toString());
      image.setAttribute("y", (parseFloat(cy) - 10).toString());
...
class Graph {
  ...
  public loadCommits(
    ...
    if (
      ...
      this.config.uncommittedChanges ===
        GG.GraphUncommittedChangesStyle.OpenCircleAtTheUncommittedChanges
    ) {
  ...
  public getWidthsAtVertices() {
    ...
    for (i = 0; i < this.vertices.length; i++) {
      widths[i] =
        this.config.grid.offsetX +
        this.vertices[i].getNextPoint().x * this.config.grid.x -
        2;
    }
    return widths;
  }
      ...
      for (let i = 0; i < this.commits.length; i++) {
        if (
          ...
          (this.commits[i].stash === null ||
            typeof this.commitLookup[this.commits[i].stash!.baseHash] !==
              "number" ||
  ...
  public getFirstChildIndex(i: number) {
    ...
    if (children.length > 1) {
      ...
      if (
        branch !== null &&
        (childOnSameBranch = children.find((child) =>
          child.isOnThisBranch(branch)
        ))
      ) {
  ...
  public getAlternativeChildIndex(i: number) {
    ...
    if (children.length > 1) {
      ...
      if (
        ...
        return Math.max(
          ...children
            .filter((child) => child !== childOnSameBranch)
            .map((child) => child.id)
        );
      } else {
        // No child could be found on the same branch as the vertex
  ...
  private setSvgWidth(contentWidth: number) {
    let width =
      this.maxWidth > -1 ? Math.min(contentWidth, this.maxWidth) : contentWidth;
  ...
  private vertexOver(event: MouseEvent) {
    ...
    if (id < this.commits.length && this.commits[id].hash !== UNCOMMITTED) {
      ...
      this.tooltipTimeout = setTimeout(() => {
        ...
        if (
          vertexScreenY >= 5 &&
          vertexScreenY <= this.viewElem.clientHeight - 5
        ) {
  ...
  private showTooltip(id: number, vertexScreenY: number) {
    if (this.tooltipVertex !== null) {
      this.tooltipVertex.setAttribute(
        "r",
        this.tooltipVertex.classList.contains("stashOuter") ? "5.5" : "5"
      );
    }
    ...
    let html =
      '<div class="graphTooltipTitle">Commit ' +
      abbrevCommit(this.commits[id].hash) +
      "</div>";
    if (
    ...
    if (heads.length > 0 || remotes.length > 0) {
      ...
      branchLabels.heads.forEach((head) => {
        let html = head.remotes.reduce(
          (prev, remote) =>
            prev +
            '<span class="graphTooltipCombinedRef">' +
            escapeHtml(remote) +
            "</span>",
          ""
        );
        htmlRefs.push(
          '<span class="graphTooltipRef">' +
      ...
      branchLabels.remotes.forEach((remote) =>
        htmlRefs.push(
          '<span class="graphTooltipRef">' + escapeHtml(remote.name) + "</span>"
        )
      );
      html +=
        '<div class="graphTooltipSection">Branches: ' +
        getLimitedRefs(htmlRefs) +
        "</div>";
    }
    if (tags.length > 0) {
      let htmlRefs = tags.map(
        (tag) => '<span class="graphTooltipRef">' + escapeHtml(tag) + "</span>"
      );
      html +=
        '<div class="graphTooltipSection">Tags: ' +
        getLimitedRefs(htmlRefs) +
        "</div>";
    }
    if (stashes.length > 0) {
      let htmlRefs = stashes.map(
        (stash) =>
          '<span class="graphTooltipRef">' + escapeHtml(stash) + "</span>"
      );
      html +=
        '<div class="graphTooltipSection">Stashes: ' +
        getLimitedRefs(htmlRefs) +
        "</div>";
    }
    ...
    const point = this.vertices[id].getPoint(),
      color =
        "var(--git-graph-color" +
        (this.vertices[id].getColour() % this.config.colours.length) +
        ")";
    ...
    const pixel: Pixel = {
      ...
      y:
        ...
        this.config.grid.offsetY +
        (this.expandedCommitIndex > -1 && id > this.expandedCommitIndex
    ...
    content.style.maxWidth =
      Math.min(
        this.contentElem.getBoundingClientRect().width - pixel.x - 35,
        600
      ) + "px"; // Tooltip Offset [23px] + Tooltip Border [2 * 2px] + Right Page Margin [8px] = 35px
    ...
    if (
      vertexScreenY + relativeOffset + tooltipRect.height >
      this.viewElem.clientHeight - 4
    ) {
      // Not enough height below the vertex to fit the vertex, shift it up.
      relativeOffset =
        this.viewElem.clientHeight - vertexScreenY - 4 - tooltipRect.height;
    }
  ...
  private closeTooltip() {
    ...
    if (this.tooltipVertex !== null) {
      this.tooltipVertex.setAttribute(
        "r",
        this.tooltipVertex.classList.contains("stashOuter") ? "4.5" : "4"
      );
