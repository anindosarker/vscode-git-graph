class GitGraphView {
  ...
  constructor(viewElem: HTMLElement, prevState: WebViewState | null) {
  	...
  	this.scrollShadowElem = <HTMLInputElement>(
      document.getElementById('scrollShadow')!
    );
  	...
  	this.repoDropdown = new Dropdown(
  		'repoDropdown',
  		true,
  		false,
  		'Repos',
  	...
  	if (
  		!this.loadRepos(this.gitRepos, initialState.lastActiveRepo, loadViewTo)
  	) {
  		if (prevState) {
  	...
  	fetchBtn.title =
      'Fetch' +
      (this.config.fetchAndPrune ? ' & Prune' : '') +
      ' from Remote(s)';
  		...
  		runAction(
  			{
  				...
  				name:
            this.gitRepos[this.currentRepo].name ||
            getRepoName(this.currentRepo)
  			},
  			'Opening Terminal'
  		);
  	});
  ...
  public loadRepos(
  	...
  	if (loadViewTo !== null) {
  		if (loadViewTo.repo === newRepo) {
  			...
  			showErrorMessage(
  				'Unable to load the Git Graph View for the repository "' +
            loadViewTo.repo +
            '". It is not currently included in Git Graph.'
  			);
  		}
  	} else {
  		this.loadViewTo = null;
  ...
  private loadRepoInfo(
  	...
  	if (
  		this.currentBranches !== null &&
      !(
      	this.currentBranches.length === 1 &&
        this.currentBranches[0] === SHOW_ALL_BRANCHES
      )
  	) {
  	...
  	this.branchDropdown.setOptions(
  		this.getBranchOptions(true),
  		this.currentBranches
  	);

  	// Remove hidden remotes that no longer exist
  	let hiddenRemotes = this.gitRepos[this.currentRepo].hideRemotes;
  	let hideRemotes = hiddenRemotes.filter((hiddenRemote) =>
  ...
  private finaliseLoadRepoInfo(repoInfoChanges: boolean, isRepo: boolean) {
  	...
  	if (refreshState.inProgress) {
  		if (isRepo) {
  			refreshState.repoInfoChanges =
          refreshState.repoInfoChanges || repoInfoChanges;
  ...
  private loadCommits(
      ...
      arraysEqual(
          ...
          arraysEqual(
          	a.tags,
          	b.tags,
          	(a, b) => a.name === b.name && a.annotated === b.annotated
          ) &&
          arraysEqual(
          	a.remotes,
          	b.remotes,
          	(a, b) => a.name === b.name && a.remote === b.remote
          ) &&
          arraysStrictlyEqual(a.parents, b.parents) &&
          ((a.stash === null && b.stash === null) ||
            (a.stash !== null &&
              b.stash !== null &&
              a.stash.selector === b.stash.selector))
      ) &&
      this.renderedGitBranchHead === this.gitBranchHead
          ...
          this.expandedCommit.commitElem !== null
  			) {
      ...
      (!expandedCommitVisible ||
        (this.expandedCommit.compareWithHash !== null &&
          !expandedCompareWithCommitVisible))
  	) {
  		this.closeCommitDetails(false);
  	}
  	...
  	if (
  		currentRepoLoading &&
      this.config.onRepoLoad.scrollToHead &&
  ...
  private finaliseRepoLoad(didLoadRepoData: boolean) {
  	if (this.loadViewTo !== null && this.currentRepo === this.loadViewTo.repo) {
        ...
        (this.expandedCommit === null ||
          this.expandedCommit.commitHash !==
            this.loadViewTo.commitDetails.commitHash ||
          this.expandedCommit.compareWithHash !==
            this.loadViewTo.commitDetails.compareWithHash)
  		) {
  			const commitIndex = this.getCommitId(
  				this.loadViewTo.commitDetails.commitHash
  			);
          ...
          this.loadViewTo.commitDetails.compareWithHash !== null
  			...
  			const compareWithElem = findCommitElemWithId(
  				commitElems,
  				compareWithIndex
  			);
          ...
          (this.loadViewTo.commitDetails.compareWithHash === null ||
            compareWithElem !== null)
  				...
  				showErrorMessage(
  					'Unable to resume Code Review, it could not be found in the latest ' +
              this.maxCommits +
              ' commits that were loaded in this repository.'
  				);
  			}
  ...
  public processLoadRepoInfoResponse(msg: GG.ResponseLoadRepoInfo) {
  	if (msg.error === null) {
        ...
        refreshState.loadRepoInfoRefreshId === msg.refreshId
  		) {
  			this.loadRepoInfo(
  				msg.branches,
  				msg.head,
  ...
  public processLoadCommitsResponse(msg: GG.ResponseLoadCommits) {
  	if (msg.error === null) {
  		...
  		if (
  			refreshState.inProgress &&
        refreshState.loadCommitsRefreshId === msg.refreshId
  		) {
  			this.loadCommits(
  				msg.commits,
  				msg.head,
        ...
        msg.error.indexOf('bad revision \'HEAD\'') > -1
  ...
  public loadAvatar(email: string, image: string) {
  	...
  	this.saveState();
  	let avatarsElems = <HTMLCollectionOf<HTMLElement>>(
  	...
  	for (let i = 0; i < avatarsElems.length; i++) {
  		if (avatarsElems[i].dataset.email === escapedEmail) {
  			avatarsElems[i].innerHTML =
          '<img class="avatarImg" src="' + image + '">';
  		}
  	}
  	this.graph.loadAvatar(email, image);
  	this.render();
  }
  ...
  public getBranchOptions(
  	includeShowAll?: boolean
  ): ReadonlyArray<DialogSelectInputOption> {
  ...
  private getPushRemote(branch: string | null = null) {
  	...
  	if (this.gitConfig !== null) {
  		if (
  			branch !== null &&
        typeof this.gitConfig.branches[branch] !== 'undefined'
  		) {
  	...
  	return (
  		possibleRemotes.find(
  			(remote) => remote !== null && this.gitRemotes.includes(remote)
  		) || this.gitRemotes[0]
  	);
  }
  ...
  private requestLoadRepoInfoAndCommits(
  	...
  	if (this.commits.length === 0) {
  		this.tableElem.innerHTML =
        '<h2 id="loadingHeader">' + SVG_ICONS.loading + 'Loading ...</h2>';
  	}
  ...
  public requestCommitDetails(hash: string, refresh: boolean) {
  	...
  	sendMessage({
  		...
  		stash: commit.stash,
  		avatarEmail:
        this.config.fetchAvatars && hash !== UNCOMMITTED ? commit.email : null,
  		refresh: refresh
  	});
  }

  public requestCommitComparison(
  	hash: string,
  ...
  private renderGraph() {
  	...
  	const headerHeight =
      colHeadersElem !== null ? colHeadersElem.clientHeight + 1 : 0;
  	const expandedCommit = this.isCdvDocked() ? null : this.expandedCommit;
  	const expandedCommitElem =
      expandedCommit !== null ? document.getElementById('cdv') : null;
      ...
      this.commits.length > 0 && this.tableElem.children.length > 0
          ...
          this.commits.length
      	: this.config.graph.grid.y;
  	this.config.graph.grid.offsetY =
      headerHeight + this.config.graph.grid.y / 2;
  ...
  private renderTable() {
  	...
  	const widthsAtVertices = this.config.referenceLabels
  	...
  	for (let i = 0; i < this.commits.length; i++) {
  		...
  		let message =
        '<span class="text">' +
        textFormatter.format(commit.message) +
        '</span>';
          ...
          '</span>';
            ...
            '" data-fullref="' +
            escapeHtml(
            	branchLabels.heads[j].remotes[k] +
                '/' +
                branchLabels.heads[j].name
            ) +
            '">' +
            remoteName +
            '</span>';
  	...
  	if (this.expandedCommit !== null) {
  		...
  		const commitElem = findCommitElemWithId(
  			elems,
  			this.getCommitId(expandedCommit.commitHash)
  		);
  		const compareWithElem =
        expandedCommit.compareWithHash !== null
        	? findCommitElemWithId(
        		elems,
        		this.getCommitId(expandedCommit.compareWithHash)
        	)
        	: null;
  ...
  private renderUncommittedChanges() {
    ...
    document.getElementById('uncommittedChanges')!.innerHTML =
      ...
      (colVisibility.date
      	...
      	: '') +
      (colVisibility.author
      	? '<td class="authorCol text" title="* <>">*</td>'
      	: '') +
  ...
  public renderRefreshButton() {
  	...
  	this.refreshBtnElem.innerHTML = enabled
  		? SVG_ICONS.refresh
  		: SVG_ICONS.loading;
  	alterClass(this.refreshBtnElem, CLASS_REFRESHING, !enabled);
  }

  public renderTagDetails(
  	tagName: string,
  	commitHash: string,
  	...
  	dialog.showMessage(
        ...
        (details.signature !== null
        	? generateSignatureHtml(details.signature)
        	: '') +
        '<br>' +
        '<b>Date: </b>' +
  ...
  public renderRepoDropdownOptions(repo?: string) {
  	this.repoDropdown.setOptions(getRepoDropdownOptions(this.gitRepos), [
  		repo || this.currentRepo
  	]);
  }
  	...
  	return [
  		[
  			...
  			},
  			{
  				...
  				onClick: () => {
  					dialog.showRefInput(
  						'Enter the new name for branch <b><i>' +
                escapeHtml(refName) +
                '</i></b>:',
  						refName,
  						'Rename Branch',
  			...
  			},
  			{
                  ...
                  (this.gitRemotes.length > 1 ? 's' : ''),
  							value: false,
  							info:
                  'This branch is on the remote' +
                  (remotesWithBranch.length > 1 ? 's: ' : ' ') +
                  formatCommaSeparatedList(
                  	remotesWithBranch.map((remote) => '"' + remote + '"')
                  )
  						});
  					}
  					dialog.showForm(
  							...
  							runAction(
  								{
  									...
  									deleteOnRemotes:
                      remotesWithBranch.length > 0 && <boolean>values[1]
                      	? remotesWithBranch
                      	: []
  								},
  								'Deleting Branch'
  			...
  			},
  			{
  				...
  				onClick: () =>
  					this.mergeAction(refName, refName, GG.MergeActionOn.Branch, target)
  			},
  			{
  			...
  			},
  			{
  				...
  				onClick: () => {
  					...
  					if (multipleRemotes) {
  						inputs.unshift({
  							...
  							options: this.gitRemotes.map((remote) => ({
  								name: remote,
  								value: remote
  							})),
  							multiple: true
                ...
                '?',
                      ...
                      remotes.length > 0 &&
                      (this.gitConfig === null ||
                        typeof this.gitConfig.branches[refName] ===
                          'undefined' ||
                        this.gitConfig.branches[refName].remote !==
  		...
  		[
            ...
            this.gitRepos[this.currentRepo].pullRequestConfig !== null,
  				onClick: () => {
  					...
  					dialog.showCheckbox(
  						'Are you sure you want to create a Pull Request for branch <b><i>' +
                escapeHtml(refName) +
                '</i></b>?',
  ...
  private getCommitContextMenuActions(
  	...
  	return [
  		[
  			{
  				...
  				onClick: () =>
  					this.addTagAction(
  						hash,
  						'',
  						this.config.dialogDefaults.addTag.type,
  						'',
  						null,
  						target
  					)
            ...
            (globalState.alwaysAcceptCheckoutCommit ? '' : ELLIPSIS),
  			...
  			},
  			{
                ...
                '</i></b>?',
  						...
  						'Yes, cherry pick',
  						(values) => {
  							let parentIndex = isMerge
  								? parseInt(<string>values.shift())
  								: 0;
  							runAction(
  								{
  			...
  			},
  			{
  				...
  				visible:
            visibility.drop &&
            this.graph.dropCommitPossible(this.commitLookup[hash]),
  				onClick: () => {
  					dialog.showConfirmation(
  		...
  		[
  			...
  			},
  			{
                ...
                (this.gitBranchHead !== null
                	? '<b><i>' +
                    escapeHtml(this.gitBranchHead) +
                    '</i></b> (the current branch)'
                	: 'the current branch') +
                ' to commit <b><i>' +
                abbrevCommit(hash) +
                '</i></b>?',
  						...
  						[
  							{
  								...
  								value: GG.GitResetMode.Soft
  							},
  							{
  								name: 'Mixed - Keep working tree, but reset index',
  								value: GG.GitResetMode.Mixed
  							},
  							{
  								name: 'Hard - Discard all changes',
  								value: GG.GitResetMode.Hard
  							}
  						],
  						'Yes, reset',
  						(mode) => {
  ...
  private getRemoteBranchContextMenuActions(
  	...
  	const isSelectedInBranchesDropdown =
      this.branchDropdown.isSelected(prefixedRefName);
  	return [
  		[
  			{
  				...
  				visible: visibility.checkout,
  				onClick: () =>
  					this.checkoutBranchAction(refName, remote, null, target)
  			},
  			{
  				...
  				onClick: () => {
  					dialog.showConfirmation(
  						'Are you sure you want to delete the remote branch <b><i>' +
                escapeHtml(refName) +
                '</i></b>?',
  						'Yes, delete',
  						() => {
  						...
  						[
  							{
  								...
  								name: 'Force Fetch',
  								value:
                    this.config.dialogDefaults.fetchIntoLocalBranch.forceFetch,
  			...
  			},
  			{
  				...
  				onClick: () =>
  					this.mergeAction(
  						refName,
  						refName,
  						GG.MergeActionOn.RemoteTrackingBranch,
  						target
  					)
  			},
  			{
  		...
  		[
            ...
            this.gitRepos[this.currentRepo].pullRequestConfig !== null &&
            branchName !== 'HEAD' &&
            (this.gitRepos[this.currentRepo].pullRequestConfig!.sourceRemote ===
              remote ||
              this.gitRepos[this.currentRepo].pullRequestConfig!.destRemote ===
                remote),
  				onClick: () => {
  					const config = this.gitRepos[this.currentRepo].pullRequestConfig;
  					if (config === null) return;
  					...
  					runAction(
  						{
  							...
  							repo: this.currentRepo,
  							config: config,
  							sourceRemote: isDestRemote
  								? config.destRemote!
  								: config.sourceRemote,
  							sourceOwner: isDestRemote
  								? config.destOwner
  								: config.sourceOwner,
  ...
  private getStashContextMenuActions(
  	target: DialogTarget & RefTarget
  ): ContextMenuActions {
  	const hash = target.hash,
  		selector = target.ref,
  		...
  		[
  			{
  				...
  				onClick: () => {
  					dialog.showForm(
  						'Are you sure you want to apply the stash <b><i>' +
                escapeHtml(selector.substring(5)) +
                '</i></b>?',
  						[
  							{
  								type: DialogInputType.Checkbox,
  			...
  			},
  			{
  				...
  				onClick: () => {
  					dialog.showRefInput(
  						'Create a branch from stash <b><i>' +
                escapeHtml(selector.substring(5)) +
                '</i></b> with the name:',
  						'',
  						'Create Branch',
  			...
  			},
  			{
  				...
  				onClick: () => {
  					dialog.showForm(
  						'Are you sure you want to pop the stash <b><i>' +
                escapeHtml(selector.substring(5)) +
                '</i></b>?',
  						[
  							{
  			...
  			},
  			{
  				...
  				onClick: () => {
  					dialog.showConfirmation(
  						'Are you sure you want to drop the stash <b><i>' +
                escapeHtml(selector.substring(5)) +
                '</i></b>?',
  						'Yes, drop',
  						() => {
  		...
  		[
              ...
              '</i></b>?';
  					if (this.gitRemotes.length > 1) {
  						let options = [
  							{ name: 'Don\'t delete on any remote', value: '-1' }
  						];
  						...
  						dialog.showSelect(
  							...
  							(remoteIndex) => {
  								this.deleteTagAction(
  									tagName,
  									remoteIndex !== '-1'
  										? this.gitRemotes[parseInt(remoteIndex)]
  										: null
  								);
  							},
  							target
  						...
  						dialog.showCheckbox(
  							...
  							(deleteOnRemote) => {
  								this.deleteTagAction(
  									tagName,
  									deleteOnRemote ? this.gitRemotes[0] : null
  								);
  							},
  							target
  						);
  			...
  			},
  			{
                  ...
                  '</i></b>?',
  						...
  						const options = this.gitRemotes.map((remote) => ({
  							name: remote,
  							value: remote
  						}));
  						dialog.showMultiSelect(
  							'Are you sure you want to push the tag <b><i>' +
                  escapeHtml(tagName) +
                  '</i></b>? Select the remote(s) to push the tag to:',
  							defaults,
  							options,
  							'Yes, push',
  							(remotes) => {
  ...
  private getUncommittedChangesContextMenuActions(
  	...
  	let visibility =
      this.config.contextMenuActionsVisibility.uncommittedChanges;
  	return [
  		[
  			{
  					...
  					dialog.showForm(
  						...
  						[
  							...
  							},
  							{
  								...
  								value:
                    this.config.dialogDefaults.stashUncommittedChanges
                    	.includeUntracked,
  ...
  private addTagAction(
      ...
      mostRecentTagsIndex > -1
      	? this.commits[mostRecentTagsIndex].tags.map(
      		(tag) => '"' + tag.name + '"'
      	)
      	: [];
  	...
  	const inputs: DialogInput[] = [
  		...
  		},
  		{
  			...
  			name: 'Type',
  			default:
          initialType === GG.TagType.Annotated ? 'annotated' : 'lightweight',
  			options: [
          ...
          this.gitRemotes.length > 1 && <string>values[3] !== '-1'
  			...
  			if (this.gitTags.includes(tagName)) {
  				dialog.showTwoButtons(
  					'A tag named <b><i>' +
              escapeHtml(tagName) +
              '</i></b> already exists, do you want to replace it with this new tag?',
  							...
  							runAction(
  								{
  									...
  									pullAfterwards: canPullFromRemote
  										? {
  											...
  											remote: remote,
  											createNewCommit:
                            this.config.dialogDefaults.pullBranch.noFastForward,
  											squash: this.config.dialogDefaults.pullBranch.squash
  										}
  										: null
  								},
  								'Checking out Branch' +
                    (canPullFromRemote ? ' & Pulling Changes' : '')
  							);
  						},
  						target
  					);
  		...
  		(values) => {
  			...
  			if (this.gitBranches.includes(branchName)) {
  				dialog.showTwoButtons(
  					'A branch named <b><i>' +
              escapeHtml(branchName) +
              '</i></b> already exists, do you want to replace it with this new branch?',
  ...
  private makeTableResizable() {
  	let colHeadersElem = document.getElementById('tableColHeaders')!,
        ...
        document.getElementsByClassName('tableColHeader')
      );
  	...
  	const makeTableFixedLayout = () => {
  		...
  		for (let i = 2; i < cols.length; i++) {
  			cols[i].style.width =
          columnWidths[parseInt(cols[i].dataset.col!)] + 'px';
  		}
  	...
  	for (let i = 0; i < cols.length; i++) {
  		...
  		cols[i].innerHTML +=
        (i > 0
        	? '<span class="resizeCol left" data-col="' + (col - 1) + '"></span>'
        	: '') +
        (i < cols.length - 1
        	? '<span class="resizeCol right" data-col="' + col + '"></span>'
        	: '');
  	}
  	...
  	if (columnWidths[0] !== COLUMN_AUTO) {
  		...
  		if (Math.max(graphWidth, colWidth) > maxWidth) {
  			...
  			this.tableElem.style.setProperty(
          ...
          '6px ' +
          Math.floor(
          	(Math.max(graphWidth, 64) -
              (colWidth - COLUMN_LEFT_RIGHT_PADDING)) /
              2
          ) +
          'px';
  		}
  	...
  	const processResizingColumn: EventListener = (e) => {
  		if (col > -1) {
  			...
  			if (col === 0) {
  				...
  				if (
  					cols[1].clientWidth - COLUMN_LEFT_RIGHT_PADDING - mouseDeltaX <
            COLUMN_MIN_WIDTH
  				)
  					mouseDeltaX =
              cols[1].clientWidth -
  	...
  	addListenerToClass('resizeCol', 'mousedown', (e) => {
  		...
  		for (let i = 0; i < cols.length; i++) {
  			...
  			if (isAuto && curCol !== 1)
  				columnWidths[curCol] =
            cols[i].clientWidth - COLUMN_LEFT_RIGHT_PADDING;
  			if (curCol === col) colIndex = i;
  		}
  		if (isAuto) makeTableFixedLayout();
  		eventOverlay.create(
  			'colResize',
  			processResizingColumn,
  			stopResizingColumn
  		);
  	});
  	...
  	colHeadersElem.addEventListener('contextmenu', (e: MouseEvent) => {
  		...
  		const commitOrdering = getCommitOrdering(
  			this.gitRepos[this.currentRepo].commitOrdering
  		);
  		const changeCommitOrdering = (
  			repoCommitOrdering: GG.RepoCommitOrdering
  		) => {
  			this.saveRepoStateValue(
  				this.currentRepo,
  				'commitOrdering',
  			...
  			[
  				...
  				[
  					...
  					},
  					{
  						...
  						onClick: () =>
  							changeCommitOrdering(GG.RepoCommitOrdering.AuthorDate)
  					},
  					{
  						...
  						onClick: () =>
  							changeCommitOrdering(GG.RepoCommitOrdering.Topological)
  					}
  				]
  			],
  			true,
  ...
  private scrollToStash(next: boolean) {
  	...
  	if (stashCommits.length > 0) {
        ...
        this.lastScrollToStash.hash !== null
        	? stashCommits.findIndex(
        		(commit) => commit.hash === this.lastScrollToStash.hash
        	)
        	: -1;
  		let scrollToStashCommitIndex =
        lastScrollToStashCommitIndex + (next ? 1 : -1);
  		...
  		this.scrollToCommit(
  			stashCommits[scrollToStashCommitIndex].hash,
  			true,
  			true
  		);
  ...
  public scrollToCommit(
      ...
      elemTop + 32 - this.viewElem.clientHeight > this.viewElem.scrollTop
  		...
  		this.viewElem.scroll(
  			0,
  			this.controlsElem.clientHeight +
          elem.offsetTop +
          12 -
          this.viewElem.clientHeight / 2
  		);
  	}
  ...
  private observeWebviewStyleChanges() {
  	let fontFamily = getVSCodeStyle(CSS_PROP_FONT_FAMILY),
  		...
  		findMatchColour = getVSCodeStyle(
  			CSS_PROP_FIND_MATCH_HIGHLIGHT_BACKGROUND
  		),
  		selectionBackgroundColor = !!getVSCodeStyle(
  			CSS_PROP_SELECTION_BACKGROUND
  		);

  	const setFlashColour = (colour: string) => {
  		document.body.style.setProperty(
  			'--git-graph-flashPrimary',
  			modifyColourOpacity(colour, 0.7)
  		);
  		document.body.style.setProperty(
        ...
        (e.key === 'ArrowUp' || e.key === 'ArrowDown')
  			...
  			if (e.ctrlKey || e.metaKey) {
  				...
  				if (e.key === 'ArrowUp' && curHashIndex > 0) {
  					...
  					e.key === 'ArrowDown' &&
            curHashIndex < this.commits.length - 1
  				) {
  					newHashIndex = curHashIndex + 1;
  				}
  			}
  		...
  		if (e.target !== null && isInternalUrlElem(<Element>e.target)) {
              ...
              (this.expandedCommit === null ||
                ...
                this.expandedCommit.compareWithHash !== null)
  						...
  						const elem = findCommitElemWithId(
  							getCommitElems(),
  							this.commitLookup[value]
  						);
  						if (elem !== null) this.loadCommitDetails(elem);
  					}
  					break;
  	...
  	document.body.addEventListener('contextmenu', (e: MouseEvent) => {
          ...
          eventTarget.closest('#cdv') !== null
  			...
  			contextMenu.show(
  				...
  				() => {
  					if (
  						target.type === TargetType.CommitDetailsView &&
              this.expandedCommit !== null
  					) {
  ...
  private observeTableEvents() {
  	...
  	this.tableElem.addEventListener('dblclick', (e: MouseEvent) => {
          ...
          eventElem.classList.contains(CLASS_REF_REMOTE)
  				...
  				let refName = unescapeHtml(eventElem.dataset.name!),
  					...
  					isRemoteCombinedWithHead =
              eventTarget.classList.contains('gitRefHeadRemote');
  	...
  	this.tableElem.addEventListener('contextmenu', (e: Event) => {
  		...
  		if ((eventElem = eventTarget.closest('.gitRef')) !== null) {
              ...
              eventTarget.classList.contains('gitRefHeadRemote');
  				if (isHead && isRemoteCombinedWithHead) {
  					target.ref = unescapeHtml(
              (<HTMLElement>eventTarget).dataset.fullref!
  					);
              ...
              (isRemoteCombinedWithHead ? <HTMLElement>eventTarget : eventElem)
              	.dataset.remote!
  					);
  ...
  public closeCommitDetails(saveAndRender: boolean) {
  	...
  	if (expandedCommit.compareWithElem !== null) {
  		expandedCommit.compareWithElem.classList.remove(
  ...
  public showCommitDetails(
  	...
  	if (
  		haveFilesChanged(expandedCommit.fileChanges, commitDetails.fileChanges)
  	) {
  		...
  		for (j = 0; j < path.length; j++) {
  			...
  			if (typeof this.gitRepos[absPath] !== 'undefined') {
  				...
  				cur.contents[path[j]] = {
  					...
  					reviewed:
              codeReview === null ||
              !codeReview.remainingFiles.includes(gitFiles[i].newFilePath)
  				};
  			}
  		}
  	}
  ...
  private loadCommitComparison(
  	commitElem: HTMLElement,
  	compareWithElem: HTMLElement
  ) {
  	const commit = this.getCommitOfElem(commitElem);
  	...
  	if (commit !== null && compareWithCommit !== null) {
  		if (this.expandedCommit !== null) {
  			if (this.expandedCommit.commitHash !== commit.hash) {
  				...
  				this.expandedCommit.compareWithHash !== compareWithCommit.hash
  			) {
  ...
  private renderCommitDetailsView(refresh: boolean) {
  	...
  	const codeReviewPossible =
      !expandedCommit.loading && commitOrder.to !== UNCOMMITTED;
  	const externalDiffPossible =
      !expandedCommit.loading &&
      (expandedCommit.compareWithHash !== null ||
        this.commits[this.commitLookup[expandedCommit.commitHash]].parents
        	.length > 0);
        ...
        ' ...</div>';
  		...
  		if (expandedCommit.compareWithHash === null) {
            ...
            (expandedCommit.avatar !== null
            	? '<span class="cdvSummaryAvatar"><img src="' +
                expandedCommit.avatar +
                '"></span>'
            	: '') +
            '</span></span><br><br>' +
      ...
      '</div>' +
      (codeReviewPossible
      	? '<div id="cdvCodeReview" class="cdvControlBtn">' +
          SVG_ICONS.review +
          '</div>'
      	: '') +
      (!expandedCommit.loading
      ...
      (externalDiffPossible
      	? '<div id="cdvExternalDiff" class="cdvControlBtn">' +
  	...
  	if (!refresh) {
  		if (isDocked) {
  			let elemTop =
          this.controlsElem.clientHeight + expandedCommit.commitElem.offsetTop;
  			if (elemTop - 8 < this.viewElem.scrollTop) {
  				// Commit is above what is visible on screen
  				this.viewElem.scroll(0, elemTop - 8);
  			} else if (
  				elemTop - this.viewElem.clientHeight + 32 >
          this.viewElem.scrollTop
  			) {
  			...
  			if (this.config.commitDetailsView.autoCenter) {
  				...
  				this.viewElem.scroll(
  					0,
  					elemTop - 12 + (cdvHeight - this.viewElem.clientHeight) / 2
  				);
  				...
  				elemTop + cdvHeight - this.viewElem.clientHeight + 8 >
          this.viewElem.scrollTop
  			) {
    ...
    if (!expandedCommit.loading) {
      ...
      document
      	.getElementById('cdvFileViewTypeTree')!
      	.addEventListener('click', () => {
      		this.changeFileViewType(GG.FileViewType.Tree);
      	});

      document
      	.getElementById('cdvFileViewTypeList')!
      	.addEventListener('click', () => {
      ...
      if (externalDiffPossible) {
        document
        	.getElementById('cdvExternalDiff')!
        	.addEventListener('click', () => {
        		const expandedCommit = this.expandedCommit;
        		if (
        			expandedCommit === null ||
              this.gitConfig === null ||
              (this.gitConfig.diffTool === null &&
  ...
  private setCdvDivider() {
  	let percent =
      (this.gitRepos[this.currentRepo].cdvDivider * 100).toFixed(2) + '%';
  ...
  private makeCdvResizable() {
  	...
  	const processResizingCdvHeight: EventListener = (e) => {
  		...
  		let height =
        this.gitRepos[this.currentRepo].cdvHeight + (isDocked ? -delta : delta);
  	...
  	addListenerToClass('cdvHeightResize', 'mousedown', (e) => {
  		...
  		eventOverlay.create(
  			'rowResize',
  			processResizingCdvHeight,
  			stopResizingCdvHeight
  		);
  	});
  }
  ...
  private makeCdvDividerDraggable() {
    ...
    document.getElementById('cdvDivider')!.addEventListener('mousedown', () => {
    	...
    	eventOverlay.create(
    		'colResize',
    		processDraggingCdvDivider,
    		stopDraggingCdvDivider
    	);
    });
  }
  ...
  private cdvUpdateFileState(
  	...
  	if (fileWasViewed) {
  		...
  		insertBeforeFirstChildWithClass(
  			lastViewedElem,
  			fileElem,
  			'fileTreeFileAction'
  		);
  	...
  	if (expandedCommit.codeReview !== null) {
  		if (isReviewed !== null) {
            ...
            expandedCommit.codeReview.remainingFiles.filter(
  ...
  private isCdvDocked() {
  	return (
  		this.config.commitDetailsView.location ===
      GG.CommitDetailsViewLocation.DockedToBottom
  	);
  }
  ...
  private getFileViewType() {
  	return this.gitRepos[this.currentRepo].fileViewType ===
      GG.FileViewType.Default
  ...
  private makeCdvFileViewInteractive() {
  	...
  	const triggerResetFileToRevision = (
  		file: GG.GitFileChange,
  		fileElem: HTMLElement
  	) => {
  		const expandedCommit = this.expandedCommit;
  	...
  	const triggerViewFileAtRevision = (
  		file: GG.GitFileChange,
  		fileElem: HTMLElement
  	) => {
  		const expandedCommit = this.expandedCommit;
  	...
  	const triggerViewFileDiffWithWorkingFile = (
  		file: GG.GitFileChange,
  		fileElem: HTMLElement
  	) => {
  		const expandedCommit = this.expandedCommit;
  	...
  	addListenerToClass('fileTreeFolder', 'click', (e) => {
        ...
        e.target === null
  		...
  		let sourceElem = <HTMLElement>(
        (<Element>e.target).closest('.fileTreeFolder')
      );
  	...
  	addListenerToClass('fileTreeRepo', 'click', (e) => {
          ...
          (<HTMLElement>(<Element>e.target).closest('.fileTreeRepo')).dataset
          	.path!
  			)
  		});
  	});
  	...
  	addListenerToClass('fileTreeFile', 'click', (e) => {
        ...
        e.target === null
  		...
  		const sourceElem = <HTMLElement>(
          (<Element>e.target).closest('.fileTreeFile')
        ),
  			fileElem = getFileElemOfEventTarget(e.target);
  		if (!sourceElem.classList.contains('gitDiffPossible')) return;
  		triggerViewFileDiff(
  			getFileOfFileElem(expandedCommit.fileChanges, fileElem),
  			fileElem
  		);
  	});
  	...
  	addListenerToClass('fileTreeFileRecord', 'contextmenu', (e: Event) => {
        ...
        e.target === null
  		...
  		GitGraphView.closeCdvContextMenuIfOpen(expandedCommit);
  		expandedCommit.contextMenuOpen.fileView = parseInt(
        fileElem.dataset.index!
  		);
        ...
        (file.additions !== null && file.deletions !== null);
  		const fileExistsAtThisRevision =
        file.type !== GG.GitFileStatus.Deleted && !isUncommitted;
  		const fileExistsAtThisRevisionAndDiffPossible =
        fileExistsAtThisRevision && diffPossible;
  		const codeReviewInProgressAndNotReviewed =
        expandedCommit.codeReview !== null &&
        expandedCommit.codeReview.remainingFiles.includes(file.newFilePath);
  		const visibility =
        this.config.contextMenuActionsVisibility.commitDetailsViewFile;
  			...
  			[
  				[
  					...
  					},
  					{
  						...
  						visible:
                visibility.viewFileAtThisRevision &&
                fileExistsAtThisRevisionAndDiffPossible,
  						onClick: () => triggerViewFileAtRevision(file, fileElem)
  					},
  					{
  						title: 'View Diff with Working File',
  						visible:
                visibility.viewDiffWithWorkingFile &&
                fileExistsAtThisRevisionAndDiffPossible,
  					...
  					},
  					{
  						title: 'Open File',
  						visible:
                visibility.openFile && file.type !== GG.GitFileStatus.Deleted,
  				...
  				[
  					{
  						...
  						visible:
                visibility.markAsReviewed && codeReviewInProgressAndNotReviewed,
  						onClick: () =>
  							this.cdvUpdateFileState(file, fileElem, true, false)
  					},
  					{
                ...
                !codeReviewInProgressAndNotReviewed,
  						onClick: () =>
  							this.cdvUpdateFileState(file, fileElem, false, false)
  					}
  				],
  				[
  					{
  ...
  private static closeCdvContextMenuIfOpen(expandedCommit: ExpandedCommit) {
  	if (
  		expandedCommit.contextMenuOpen.summary ||
      expandedCommit.contextMenuOpen.fileView > -1
  	) {
...
window.addEventListener('load', () => {
	...
	TextFormatter.registerCustomEmojiMappings(
		initialState.config.customEmojiShortcodeMappings
	);
	...
	window.addEventListener('message', (event) => {
		...
		switch (msg.command) {
          ...
          isExtensionErrorInfo(
            ...
            (msg.pullAfterwards !== null ? ' & Pull Changes' : '')
				);
				break;
			case 'checkoutCommit':
			...
			case 'commitDetails':
				if (msg.commitDetails !== null) {
					gitGraph.showCommitDetails(
						msg.commitDetails,
						gitGraph.createFileTree(
							msg.commitDetails.fileChanges,
							msg.codeReview
						),
						msg.avatar,
						msg.codeReview,
			...
			case 'copyFilePath':
				finishOrDisplayError(
					msg.error,
					'Unable to Copy File Path to Clipboard'
				);
				break;
			...
			case 'dropStash':
				...
				break;
			case 'editRemote':
				refreshOrDisplayError(
					msg.error,
					'Unable to Save Changes to Remote',
					true
				);
				break;
			...
			case 'exportRepoConfig':
				refreshOrDisplayError(
					msg.error,
					'Unable to Export Repository Configuration'
				);
				break;
			case 'fetch':
			...
			case 'openExternalDirDiff':
				finishOrDisplayError(
					msg.error,
					'Unable to Open External Directory Diff',
					true
				);
				break;
          ...
          isExtensionErrorInfo(
			...
			case 'setWorkspaceViewState':
				finishOrDisplayError(
					msg.error,
					'Unable to save the Workspace View State'
				);
				break;
			case 'startCodeReview':
				if (msg.error === null) {
					gitGraph.startCodeReview(
						msg.commitHash,
						msg.compareWithHash,
						msg.codeReview
					);
			...
			case 'tagDetails':
				if (msg.details !== null) {
					...
					dialog.showError(
						'Unable to retrieve Tag Details',
						msg.error,
						null,
						null
					);
			...
			case 'viewDiffWithWorkingFile':
				finishOrDisplayError(
					msg.error,
					'Unable to View Diff with Working File'
				);
				break;
			...
			case 'viewScm':
				finishOrDisplayError(
					msg.error,
					'Unable to open the Source Control View'
				);
				break;
		}
	});
	...
	function handleResponsePushTagCommitNotOnRemote(
      ...
      ' ' +
      formatCommaSeparatedList(
      	remotesNotContainingCommit.map(
      		(remote) => '<b><i>' + escapeHtml(remote) + '</i></b>'
      	)
      ) +
      '.</p>' +
   ...
   */
	function isExtensionErrorInfo(
		error: GG.ErrorInfo,
		prefix: GG.ErrorInfoExtensionPrefix
	) {
   ...
   */
	function parseExtensionErrorInfo(
		error: string,
		prefix: GG.ErrorInfoExtensionPrefix
	) {
...
function generateFileTreeHtml(
	...
	const curFolderInfo =
    topLevelFolder ||
    !initialState.config.commitDetailsView.fileTreeCompactFolders
...
function getCurrentFolderInfo(
    ...
    (child = folder.contents[keys[0]]).type === 'folder'
...
function generateFileTreeLeafHtml(
	...
	if (leaf.type === 'file') {
		...
		const textFile =
      fileTreeFile.additions !== null && fileTreeFile.deletions !== null;
		const diffPossible =
      fileTreeFile.type === GG.GitFileStatus.Untracked || textFile;
      ...
      (leaf.index === fileContextMenuOpen
      ...
      (fileTreeFile.newFilePath === lastViewedFile
      	? '<span id="cdvLastFileViewed" title="Last File Viewed">' +
          SVG_ICONS.eyeOpen +
          '</span>'
      	: '') +
...
function alterFileTreeFolderOpen(
	folder: FileTreeFolder,
	folderPath: string,
	open: boolean
) {
...
function alterFileTreeFileReviewed(
	folder: FileTreeFolder,
	filePath: string,
	reviewed: boolean
) {
...
function calcFileTreeFoldersReviewed(folder: FileTreeFolder) {
	const calc = (folder: FileTreeFolder) => {
		...
		for (let i = 0; i < keys.length; i++) {
			let cur = folder.contents[keys[i]];
			if (
				(cur.type === 'folder' && !calc(cur)) ||
        (cur.type === 'file' && !cur.reviewed)
			)
				reviewed = false;
		}
		folder.reviewed = reviewed;
...
function getRepoDropdownOptions(repos: Readonly<GG.GitRepoSet>) {
	const repoPaths = getSortedRepositoryPaths(
		repos,
		initialState.config.repoDropdownOrder
	);
	const paths: string[] = [],
		names: string[] = [],
		...
		for (let i = 0; i < ambiguousNames.length; i++) {
			...
			for (let j = 0; j < ambiguousIndexes.length; j++) {
				...
				let nextSep = paths[ambiguousIndex].lastIndexOf(
					'/',
					paths[ambiguousIndex].length -
            distinctNames[ambiguousIndex].length -
            2
				);
				if (firstSep[ambiguousIndex] < nextSep) {
					// prepend the addition path and retest
					distinctNames[ambiguousIndex] = paths[ambiguousIndex].substring(
						nextSep + 1
					);
	...
	for (let i = 0; i < repoPaths.length; i++) {
		...
		if (names[i] === distinctNames[i]) {
			...
			let hintPath = distinctNames[i].substring(
				0,
				distinctNames[i].length - names[i].length - 1
			);
			...
			// Construct the hint
			hint =
        (distinctNames[i] !== paths[i] ? '.../' : '') + hintComps.join('/');
		}
	...
	if (initialState.config.referenceLabels.combineLocalAndRemoteBranchLabels) {
		...
		for (let i = 0; i < remotes.length; i++) {
			if (remotes[i].remote !== null) {
				...
				let branchName = remotes[i].name.substring(
          remotes[i].remote!.length + 1
				);
